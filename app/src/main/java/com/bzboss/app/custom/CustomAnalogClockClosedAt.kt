...
class CustomAnalogClockClosedAt : View {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawColor(Color.parseColor("#e89e52"))
        drawCircle(canvas)
        drawCenter(canvas)
        drawNumeral(canvas)
        drawHands(canvas)
...
//        hour = if (hour > 12) hour - 12 else hour
       // hours = if (hours > 12) hours - 12 else hours
        ...
        if(hoursClosedAt!=0.0){
            drawHand(canvas, ((hoursClosedAt + c[Calendar.MINUTE] / 60) * 5f).toDouble(), true)
            drawHand(canvas, minsClosedAt, false)
        }

            //drawHand(canvas, secsClosedAt, false )
